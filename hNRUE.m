    ...
    methods (Access = public)
        ...
        function configurePhy(obj, configParam)
            ...
            if (cellConfig.DuplexMode == 0) && ((configParam.DLCarrierFreq - configParam.ULCarrierFreq) < (configParam.DLBandwidth+configParam.ULBandwidth)/2)
                error('nr5g:hNRUE:InsufficientDuplexSpacing', 'DL carrier frequency must be higher than UL carrier frequency by %d MHz for FDD mode', 1e-6*(configParam.DLBandwidth+configParam.ULBandwidth)/2)
            elseif cellConfig.DuplexMode && (configParam.DLCarrierFreq ~= configParam.ULCarrierFreq)
                error('nr5g:hNRUE:InvalidCarrierFrequency', 'DL and UL carrier frequencies must have the same value for TDD mode')
            end
